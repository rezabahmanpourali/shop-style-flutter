...
import 'package:shop_style/home/data/repository/home_repository.dart'; // اضافه کردن این import
...
class _ShowModelLocationState extends State<ShowModelLocation> {
  ...
  final HomeRepository homeRepository =
      HomeRepository(); // ریپازیتوری برای ارسال مختصات
  ...
  // متد ارسال مختصات به سرور
  Future<void> _sendLatLongToServer() async {
    if (tappedLocation != null) {
      // شما باید customerId و id را از جایی گرفته باشید. فرض می‌کنیم که آنها از یک متغیر یا API گرفته می‌شوند.
      int customerId = 123; // این باید از جایی گرفته شود
      int id = 451; // این هم باید از جایی گرفته شود
      ResponseModel response = await homeRepository.sendLatLong(
      ...
      // بررسی وضعیت پاسخ و انجام کارهای لازم
      if (response.statusCode == 200) {
        // ارسال موفقیت‌آمیز بود
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("موقعیت با موفقیت ارسال شد")),
        );
      } else {
        // خطا در ارسال
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("خطا در ارسال موقعیت")),
        );
      }
    }
  }
